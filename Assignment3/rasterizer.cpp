...
void rst::rasterizer::rasterize_triangle(const Triangle& triangle, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    auto l = std::min({ v[0].x(), v[1].x(), v[2].x() }); // left
    auto r = std::max({ v[0].x(), v[1].x(), v[2].x() }); // right
    ...
    for (int x = l; x < r; x++) {
        for (int y = b; y < t; y++) {
            if (insideTriangle(x, y, triangle.v)) {
                ...
                if (auto& buf = depth_buf[height * y + x]; buf > zp) {
